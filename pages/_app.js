...
const NAV_ITEMS = [
  { label: "Dashboard", route: "/", icon: LuLayoutDashboard },
  { label: "Projects", route: "/projects", icon: LuFolderOpen },
  { label: "Analytics", route: "/analytics", icon: LuChartPie },
];
...
function Sidebar({ router, theme, toggleTheme, onLogout }) {
  ...
  return (
    <aside
      className={`w-[270px] h-screen shrink-0 flex flex-col border-r transition-colors duration-200 z-10 ${isDark
    ...
    >
      {/* ── Logo Header ── */}
      <div
        className={`relative shrink-0 flex items-center gap-3 px-5 py-5 border-b ${isDark ? "border-white/10" : "border-[#E5E7EB]"
          }`}
      >
        {/* Glow line at bottom */}
        ...
        <div className="flex flex-col leading-none">
          <span
            className={`text-[17px] font-bold tracking-[0.2em] ${isDark ? "text-white" : "text-black"
              }`}
          >
            SAFEPAD
          </span>
          <span
            className={`mt-1.5 text-[10px] font-semibold tracking-[0.18em] uppercase ${isDark ? "text-purple-300/90" : "text-purple-500/90"
              }`}
          >
            Software name
          </span>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.route}
              href={item.route}
              className={`group relative flex items-center gap-3 rounded-2xl px-3 py-2.5 transition-all duration-150 ${isActive
                  ? isDark
            ...
            >
              ...
              {/* Icon box */}
              <span
                className={`flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-xl transition-colors duration-150 ${isActive
              ...
              {/* Label */}
              <span
                className={`text-[14px] leading-snug font-semibold ${isActive
                    ? isDark
                      ? "text-[#EDF1F7]"
                      : "text-[#101828]"
                    : isDark
      ...
      {/* ── Footer ── */}
      <div
        className={`shrink-0 flex items-center justify-center gap-4 px-3 py-4 border-t ${isDark ? "border-[#262F3F]" : "border-[#E5E7EB]"
          }`}
      >
        {/* Theme toggle pill */}
        <button
          ...
          className={`group flex shrink-0 cursor-pointer items-center rounded-2xl p-1.5 transition-all duration-300 ease-in-out ${isDark
              ? "bg-white text-black hover:bg-white/90 shadow-sm"
              : "bg-[#101828] text-white hover:bg-[#101828]/85 shadow-sm"
            }`}
        >
          <span
            className={`flex h-8 w-8 items-center justify-center rounded-xl transition-colors ${isDark ? "bg-black/[0.06] text-purple-600" : "bg-white/[0.12] text-purple-500"
              }`}
          >
        ...
        {(() => {
          const isSettingsActive = router.pathname === "";
          return (
            <Link
              href="/settings"
              title="Settings"
              className={`group flex shrink-0 cursor-pointer items-center rounded-2xl p-1.5 transition-all duration-300 ease-in-out ${isSettingsActive
                  ? isDark
            ...
            >
              <span
                className={`flex h-8 w-8 items-center justify-center rounded-xl transition-colors duration-150 ${isSettingsActive
        ...
        >
          <span
            className={`flex h-8 w-8 items-center justify-center rounded-xl transition-colors ${isDark ? "bg-red-500/20 group-hover:bg-black/20" : "bg-white group-hover:bg-white/20 shadow-sm group-hover:shadow-none"
              }`}
          >
...
export default function MyApp({ Component, pageProps }) {
  ...
  const handleLogout = () => {
    console.log("Logout clicked - generic template");
  };
  ...
  return (
    <div
      className={`min-h-screen flex font-sans transition-colors duration-200 relative overflow-hidden ${isDark ? "bg-black text-white" : "bg-[#F7F8FA] text-[#101828]"
        }`}
    >
      <Sidebar
        router={router}
